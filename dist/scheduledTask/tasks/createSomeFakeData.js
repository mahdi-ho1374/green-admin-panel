...
const createSession_1 = __importDefault(
  require("../../helpers/order/createSession")
);
...
const createSomeFakeData = async () => {
  try {
    ...
    for (let i = 0; i < faker_1.faker.number.int({ min: 0, max: 4 }); i++) {
      ...
      const randomUsers = await user_1.default.aggregate([
        ...randomUsersPipeLine,
      ]);
    ...
    for (let i = 0; i < faker_1.faker.number.int({ min: 0, max: 1 }); i++) {
      ...
      const repliedText =
        replied === true ? faker_1.faker.lorem.sentence() : "";
