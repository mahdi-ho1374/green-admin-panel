...
const firstBuy_1 = __importDefault(
  require("../helpers/pipelines/user/firstBuy")
);
const user_2 = __importDefault(require("../helpers/pipelines/chart/user"));
const comment_2 = __importDefault(
  require("../helpers/pipelines/chart/comment")
);
...
const product_2 = __importDefault(
  require("../helpers/pipelines/chart/product")
);
const getDateRangeMatchPipeline_1 = __importDefault(
  require("../helpers/pipelines/getDateRangeMatchPipeline")
);
const combineChartData_1 = __importDefault(
  require("../helpers/pipelines/chart/combineChartData")
);
...
const getLast30DaysData = async (req, res, next) => {
  ...
  const getMatchPipeline = (previous) =>
    (0, getDateRangeMatchPipeline_1.default)(
...
const getChartData = async (req, res, next) => {
  ...
  try {
    const dateMatchPipeline = () =>
      (0, getDateRangeMatchPipeline_1.default)(thirtyDaysAgo, now);
    const users = await (0, combineChartData_1.default)(
    ...
    const sales = await (0, combineChartData_1.default)(
      order_1.default.aggregate([
        ...dateMatchPipeline(),
    ...
    const filledGapChartsData = Object.entries(chartsData).reduce(
      (result, [entry, data]) => {
        ...
        result[entry] = dates.map((date) => {
          const existingChartItem = data.find(
            (item) =>
